...
| ML evidence-chain (1.6.0 → 1.6 → 1.6b → 1.7 → 2.0 pre-reg) | `M4` | Variance Pilot (σ_PF=0.0083 locked), Baseline Comparison (6/11 candidates surviving regime gate), Sequential Probe (`p2_proceed_unchanged`, +0.0041 mean Δ-PF), Plan Hardening (decision-log + dataset-SHA validators, AF-CAND-0263 guard, CI scaffolding), and ML-P2.0 pre-registration scaffolding (EX-1–EX-10) all complete. σ_cross=0.0211, MDE_upper=0.0503 locked; verdict bands (GO≥0.10, CONDITIONAL≥0.055) pre-committed; regime thresholds frozen; HARD_CAP ceremony tool (0/2 used); E2E rehearsal PASSED 6/6 on synthetic data; 9 decision-log entries schema-validated; 628+ tests passing. UNBLOCKED for ML-P2 implementation. |
