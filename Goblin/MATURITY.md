...
| ML evidence-chain (1.6.0 → 1.6 → 1.6b → 1.7) | `M4` | Variance Pilot (σ_PF=0.0083 locked), Baseline Comparison (6/11 candidates surviving regime gate), Sequential Probe (`p2_proceed_unchanged` verdict, +0.0041 mean Δ-PF over 6 surviving candidates), and Plan Hardening (decision-log + dataset-SHA validators, AF-CAND-0263 property tests, 2 new CI lanes, glossary updates) all complete with append-only `Goblin/decisions/ml_decisions.jsonl`, sealed holdout (key outside repo), and 78 net new tests. ML-P2.0 HITL gate intentionally not breached. |
