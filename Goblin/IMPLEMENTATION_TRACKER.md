...
| ML Evolution Program | `in_progress` | Bundle A (ML-P0) + evidence-first phases ML-P1.6.0/1.6/1.6b/1.7 + ML-P2.0 pre-registration scaffolding (EX-1–EX-10) all complete as of 2026-04-20. σ_PF=0.0083; 6/11 candidates surviving regime gate; sequential features did not clear bar; ML-P2 to proceed with original architecture. Pre-registration decisions locked: DEC-ML-2.0-CANDIDATES (n=6 primary cohort), DEC-ML-2.0-TARGET (GO≥0.10 PF, CONDITIONAL≥0.055 PF, Q1 fragile rule, BCa bootstrap, Bonferroni 4-test family), DEC-ML-1.6b-A1-AUTHORIZATION (sequential CNN excluded). E2E rehearsal PASSED 6/6 on synthetic data. New tools: `tools/derive_mde.py`, `tools/gate_sensitivity.py`, `tools/verify_predictions_log_schema.py`, `tools/holdout_access_ceremony.py`, `tools/generate_synthetic_holdout.py`, `tools/run_p20_rehearsal.py`. New tests: +87 net (total 628 passing + 4 subprocess rehearsal). New CI: `ml-phase-gates.yml` (4 jobs), `holdout-ceremony.yml`, `e2e_p20_rehearsal.yml`. Sealed holdout HARD_CAP: 0/2 used. Regime thresholds frozen in `[ml_regime]` eval_gates.toml. Predictions log empty (ready for midpoint prediction when P2 architecture first produces a non-error PF). UNBLOCKED for ML-P2 implementation. |
