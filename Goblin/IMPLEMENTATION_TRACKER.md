...
| ML Evolution Program | `in_progress` | Bundle A (ML-P0) complete. ML-P1.6.0 / 1.6 / 1.6b / 1.7 (Variance Pilot → Baseline Comparison → Sequential Probe → Plan Hardening) complete as of 2026-04-20. σ_PF=0.0083 locked; 6/11 candidates survived regime gate; sequential features did NOT clear bar (mean Δ-PF +0.0041, target 0.10); ML-P2 to proceed with original architecture. New surfaces: `tools/verify_decision_log_schema.py`, `tools/verify_dataset_sha.py`, `src/agentic_forex/governance/locked_benchmark.py`, `src/agentic_forex/ml/{variance_pilot,baseline_runner,stationarity}.py`, `src/agentic_forex/features/sequential.py`, 4 driver scripts, 6 test files (+78 net tests, total 541), append-only `Goblin/decisions/ml_decisions.jsonl` (6 entries), 2 CI workflows (`ml-phase-gates.yml`, `holdout-access.yml`), sealed holdout `Goblin/holdout/ml_p2_holdout.parquet.enc` (key outside repo). PAUSED before ML-P2.0 HITL re-gate per owner instruction. |
