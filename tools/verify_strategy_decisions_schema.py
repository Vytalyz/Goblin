...
VALID_STAGES: frozenset[str] = frozenset({"S1", "S2", "S3", "S4", "S5", "S6", "S7", "RETIREMENT"})

VALID_OUTCOMES: frozenset[str] = frozenset({"pass", "fail", "pending", "retired", "promoted"})

VALID_DECIDED_BY: frozenset[str] = frozenset({"owner", "runner"})

DECISION_ID_RE = re.compile(r"^DEC-STRAT-AF-CAND-[A-Z0-9-]+-(?:S[1-7]|RETIREMENT)-[A-Z]+(?:-\d+)?$")
...
def _check_entry(idx: int, entry: dict, *, seen_ids: set[str]) -> None:
    for key in REQUIRED_TOP_LEVEL:
        if key not in entry:
            raise StrategyDecisionLogError(f"entry {idx}: missing required field '{key}'")

    decision_id = entry["decision_id"]
    if not isinstance(decision_id, str) or not DECISION_ID_RE.match(decision_id):
        raise StrategyDecisionLogError(f"entry {idx}: decision_id '{decision_id}' does not match required format")
    if decision_id in seen_ids:
        raise StrategyDecisionLogError(f"entry {idx}: duplicate decision_id '{decision_id}'")
    seen_ids.add(decision_id)

    candidate_id = entry["candidate_id"]
    if not isinstance(candidate_id, str) or not CANDIDATE_ID_RE.match(candidate_id):
        raise StrategyDecisionLogError(f"entry {idx}: candidate_id '{candidate_id}' does not match AF-CAND-* format")

    stage = entry["stage"]
    if stage not in VALID_STAGES:
        raise StrategyDecisionLogError(f"entry {idx}: stage '{stage}' not in {sorted(VALID_STAGES)}")

    outcome = entry["outcome"]
    if outcome not in VALID_OUTCOMES:
        raise StrategyDecisionLogError(f"entry {idx}: outcome '{outcome}' not in {sorted(VALID_OUTCOMES)}")

    decided_by = entry["decided_by"]
    if decided_by not in VALID_DECIDED_BY:
        raise StrategyDecisionLogError(f"entry {idx}: decided_by '{decided_by}' not in {sorted(VALID_DECIDED_BY)}")

    decided_at = entry["decided_at"]
    if not isinstance(decided_at, str) or not ISO_UTC_RE.match(decided_at):
        raise StrategyDecisionLogError(f"entry {idx}: decided_at '{decided_at}' is not ISO-8601 UTC (must end with Z)")

    rationale = entry["rationale"]
    if not isinstance(rationale, str) or len(rationale.strip()) < MIN_RATIONALE_CHARS:
        raise StrategyDecisionLogError(
            f"entry {idx}: rationale must be a string of at least {MIN_RATIONALE_CHARS} non-whitespace characters"
        )

    gate_results = entry["gate_results"]
    if not isinstance(gate_results, dict):
        raise StrategyDecisionLogError(f"entry {idx}: gate_results must be an object")
    for gate_name, gate in gate_results.items():
        if not isinstance(gate, dict):
            raise StrategyDecisionLogError(f"entry {idx}: gate_results['{gate_name}'] must be an object")
        ...
        if not isinstance(gate["passed"], bool):
            raise StrategyDecisionLogError(f"entry {idx}: gate_results['{gate_name}'].passed must be a boolean")

    evidence_uris = entry["evidence_uris"]
    if not isinstance(evidence_uris, list) or not all(isinstance(u, str) for u in evidence_uris):
        raise StrategyDecisionLogError(f"entry {idx}: evidence_uris must be a list of strings")

    next_action = entry["next_action"]
    if not isinstance(next_action, str) or not next_action.strip():
        raise StrategyDecisionLogError(f"entry {idx}: next_action must be a non-empty string")
...
def validate_lines(lines: Iterable[str]) -> int:
    ...
    for idx, raw in enumerate(lines, start=1):
        ...
        try:
            ...
            raise StrategyDecisionLogError(f"entry {idx}: invalid JSON ({exc})") from exc
