...
def check_regime_coverage(results: list[dict]) -> None:
    ...
    for regime, count in regime_counts.items():
        pct = count / n_rows
        assert pct >= min_pct, f"Regime {regime} has only {count}/{n_rows} rows ({pct:.1%} < {min_pct:.0%})"
...
def check_predictions_log(tmpdir: Path, results: list[dict]) -> None:
    ...
    midpoint_entry = {
        ...
        "predictor_attestation": ("EX-10 rehearsal script — not a real prediction, exercising schema only."),
    }
    trigger_entry = {
        ...
        "predictor_attestation": ("EX-10 rehearsal script — trigger entry exercises the commit_sha linkage field."),
    }
    ...
    result = subprocess.run([sys.executable, "-B", str(validator_path), str(pred_log)], capture_output=True, text=True)
    if result.returncode != 0:
        raise RuntimeError(f"Predictions log schema validation FAILED:\n{result.stdout}\n{result.stderr}")
...
def check_ceremony_happy_path(tmpdir: Path, results: list[dict]) -> None:
    try:
        ...
        raise RuntimeError("cryptography package required for rehearsal: pip install 'cryptography>=43,<44'")
...
def check_cap_enforcement(results: list[dict]) -> None:
    ...
    assert str(_cer.HARD_CAP) in refusal, f"Refusal message doesn't mention HARD_CAP={_cer.HARD_CAP}: {refusal}"
...
def check_decisions_log_schema(tmpdir: Path, results: list[dict]) -> None:
    ...
    if result.returncode != 0:
        raise RuntimeError(f"Decision log schema validation FAILED:\n{result.stdout}\n{result.stderr}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    report = {
        ...
        "failures": failures,
        "steps": results,
        "synthetic_holdout_sha256": hashlib.sha256(SYNTHETIC_HOLDOUT.read_bytes()).hexdigest()
        if SYNTHETIC_HOLDOUT.exists()
        else None,
        "governance_note": (
