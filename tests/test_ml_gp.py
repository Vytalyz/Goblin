...
class TestPrimitiveSet:
    ...
    def test_no_conflation_with_ea(self):
        ...
        for name in FEATURE_TERMINALS:
            ...
            assert name.lower() != "ea", (
                f"Terminal named '{name}' would cause EvA/EA conflation"
            )
...
class TestGPDiscovery:
    ...
    def test_tree_depth_capped(self):
        ...
        result = run_gp_discovery(
            X, y, population_size=10, generations=3,
            max_tree_depth=max_depth, random_seed=2,
        )
    ...
    def test_parsimony_coeff_recorded(self):
        ...
        X, y = _make_feature_matrix(200)
        result = run_gp_discovery(
            X, y, population_size=8, generations=2,
            parsimony_coefficient=0.05, random_seed=5,
        )
    ...
    def test_to_dict_keys(self):
        ...
        d = result.to_dict()
        for key in [
            "best_rule_str", "best_oos_profit_factor", "best_fitness",
            "best_tree_depth", "population_size", "generations_run",
            "parsimony_coefficient", "max_tree_depth", "converged", "top_rules",
        ]:
    ...
    def test_top_rules_list(self):
        ...
        from agentic_forex.ml.gp_rules import run_gp_discovery
        X, y = _make_feature_matrix(200)
        result = run_gp_discovery(X, y, population_size=10, generations=3,
                                  n_top_rules=3, random_seed=7)
    ...
    def test_evaluate_rule_on_features_all_win(self):
        ...
        def always_true(*_): return 1.0
        pf, wr, n = _evaluate_rule_on_features(always_true, X, y,
                                                stop_loss_pips=10.0,
